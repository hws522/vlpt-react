...
여기서 posts 배열 안의 id 가 1 인 post 객체를 찾아서, comments 에 새로운 댓글 객체를 추가해줘야 한다고 가정한다면, 다음과 같이 업데이트 해줘야 할 것이다.
...
어려운 것은 아니나, 코드의 구조가 좀 복잡해져서 코드를 봤을 때 한 눈에 들어오질 않는다.
...
우선 코드의 상단에서 `immer` 를 불러와주어야 한다. 보통 `produce` 라는 이름으로 불러온다.
...
예를들어서 우리가 만들었던 프로젝트의 상태의 경우 users 배열이 객체의 깊은곳에 위치하지 않기 때문에 새 항목을 추가하거나 제거 할 때는 `Immer` 를 사용하는 것 보다 `concat` 과 `filter` 를 사용하는것이 더 코드가 짧고 편하다.

하지만, 사용법을 잘 배워보기 위하여 해당 업데이트도 `Immer` 를 사용하여 처리를 해준다.
...
우리가 이전에 `useState` 를 사용 할 때 함수형 업데이트란걸 할 수 있다고 배웠다.
